...
def grid_refine_inner_edge(x_orig,nlev,nspan):
    ...
    for ilev in range(nlev):
        ...
        if rev:
            x = x[::-1]
    return x
def main(path, cut_indices, dens=1):
    ...
    # We take the azimuthal average without the planet region.

    i_start, i_end, j_start, j_end = cut_indices
    ...
    new_gasdens = np.outer(rho_mean, np.ones(shape=len(fargo.sigma_gas_cgs[0])))*dens
